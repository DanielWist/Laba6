...
def change_speed(n_clicks, speed):
    if n_clicks is None or speed is None:
        return "Введіть швидкість і натисніть 'Змінити швидкість'."
    try:
        speed = float(speed)
        response = update_object_speed(speed)
        return f"Швидкість об'єкта змінена на: {response.get('objectSpeed', 'невідомо')} км/г"
    except ValueError:
...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Input(id='speed-input', type='number', placeholder='Введіть швидкість (км/г)'),
